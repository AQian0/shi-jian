...
export const range = (token: FormatToken, locale = "en", genitive = false): string[] => {
  ...
  let result: string[];
  if (token === "M") {
    result = generateFormattedArray(MONTHS_PER_YEAR, i => i + 1);
  } else if (token === "MM") {
    result = generateFormattedArray(MONTHS_PER_YEAR, i => {
      const m = i + 1;
    ...
    result = generateFormattedArray(DAYS_IN_WEEK, i => `0${i + 2}`).map(d =>
    ...
    result = generateFormattedArray(YEAR_RANGE, i => i + 1).reduce(
      (ranges, i) => {
    ...
    result = generateFormattedArray(
      HOURS_PER_DAY_24H,
      i => `${token === "HH" && i <= SINGLE_DIGIT_MAX ? "0" : ""}${i}`,
    );
  } else if (token.startsWith("h")) {
    result = generateFormattedArray(
      HOURS_PER_DAY_12H,
    ...
    result = generateFormattedArray(
      MAX_MINUTE + 1,
      i => `${token.length > 1 && i <= SINGLE_DIGIT_MAX ? "0" : ""}${i}`,
    );
  } else {
    result = [];
  }
