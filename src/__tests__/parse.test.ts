...
describe("parse", () => {
  it("can parse a simple MM/DD/YYYY format", () => {
    expect(parse("10/05/2022", "MM/DD/YYYY", "en").toISOString()).toBe("2022-10-05T04:00:00.000Z");
  });
  it("can parse a format with some escapes and characters", () => {
    expect(parse("Checkin: Dec 17, 2040", "C\\heckin: MMM D, YYYY", "en").toISOString()).toBe(
      "2040-12-17T05:00:00.000Z",
    );
  });
  ...
  it("can parse space delimiters", () => {
    expect(parse("5 1 77", "M D YY").toISOString()).toBe("1977-05-01T04:00:00.000Z");
  });
  it("can parse the time of day", () => {
    ...
    expect(parse("5:22am", "h:mma").toISOString()).toBe(
      `${format(new Date(), "YYYY-MM-DDT")}${am < 10 ? "0" + am : am}:22:00.000Z`,
    );
  });
  it("can parse the time of day", () => {
    const tz = new Date().getTimezoneOffset() / 60;
  ...
  it("can parse [+-]HH:mm", () => {
    expect(parse("1994-06-22T04:22:32+09:00", "YYYY-MM-DDTHH:mm:ssZ").toISOString()).toBe(
      "1994-06-21T19:22:32.000Z",
    );
    expect(parse("1994-06-22T04:22:32+09:00").toISOString()).toBe("1994-06-21T19:22:32.000Z");
  });
  ...
  it("can parse 4pm August 15, 99", () => {
    expect(parse("4pm on August 15, 99", "ha on MMMM D, YY").toISOString()).toBe(
      "1999-08-15T20:00:00.000Z",
    );
  });
  it("can parse midnight on december 31", () => {
    expect(parse("December 31 1999, 12am", "MMMM D YYYY, ha").toISOString()).toBe(
  ...
  it("throws an error when the format does not include the time", () => {
    ...
    ).toThrowError(`Date (December 31, 1999) does not match format (MMMM D, YYYY at h:mm A)`);
  });
  ...
  it("throws an error when an incomplete iso time does not match a locale time", () => {
    ...
    ).toThrowError(`Date (2021-12-31) does not match format (MMMM D, YYYY at h:mm A)`);
  });
  ...
  it("can parse a custom format with a timezone offset", () => {
    expect(parse("2023-02-24T13:44-05:00", "YYYY-MM-DDTHH:mmZ", "en").toISOString()).toBe(
      "2023-02-24T18:44:00.000Z",
    );
    expect(parse("2023--05:00-02-24T13:44", "YYYY-Z-MM-DDTHH:mm", "en").toISOString()).toBe(
      "2023-02-24T18:44:00.000Z",
    );
    expect(parse("2023-02-24T13:44-0500", "YYYY-MM-DDTHH:mmZZ", "en").toISOString()).toBe(
      "2023-02-24T18:44:00.000Z",
    );
    expect(parse("2023--0500-02-24T13:44", "YYYY-ZZ-MM-DDTHH:mm", "en").toISOString()).toBe(
      "2023-02-24T18:44:00.000Z",
    );
  });
  ...
  it("can parse an out of range date and get the last date in that month", () => {
    expect(parse("2023-02-31", "YYYY-MM-DD").toISOString()).toBe("2023-02-28T05:00:00.000Z");
  });
