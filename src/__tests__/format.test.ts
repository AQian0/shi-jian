...
describe("format", () => {
  ...
  it("throws an error when two month format are used", () => {
    expect(() => format("2020-01-01", "MM MMMM")).toThrow(
      "Duplicate month format tokens are not allowed",
    );
  });
  ...
  it("can render us time with am/pm in chinese", () => {
    expect(format("2020-03-15T05:30:10", "h:mm:ss A", "zh")).toBe("5:30:10 上午");
    expect(format("2020-03-15T15:30:10", "h:mm:ss A", "zh")).toBe("3:30:10 下午");
  });
  it("can render a long date and short time", () => {
    expect(
      format("2100-05-03T04:04:01", {
        date: "full",
        time: "short",
      }),
    ).toBe("Monday, May 3, 2100 at 4:04 AM");
  });
  ...
  it("can include the timezone of a date", () => {
    expect(format("2023-05-05T05:30:10Z", "HH:mm:ss Z", "en")).toBe("01:30:10 -04:00");
    expect(format("2023-05-05T05:30:10Z", "HH:mm:ss ZZ", "en")).toBe("01:30:10 -0400");
  });
  ...
  it("can filter out the month part", () => {
    expect(
      format("2023-07-05T05:30:10Z", "YYYY-MM-DD", "en", false, part => part.partName !== "month"),
    ).toBe("2023--05");
  });
  it("can format with some escapes and characters", () => {
    expect(format("2040-12-17T05:00:00.000Z", "C\\heckin: MMM D, YYYY", "en")).toBe(
      "Checkin: Dec 17, 2040",
    );
  });
